...
const BuildPolygonUrl = (inputs) => {
    const baseUrlV1 = "https://api.massive.com/v1";
    const baseUrlV2 = "https://api.massive.com/v2";
    const baseUrlV3 = "https://api.massive.com/v3";
    ...
    else if (inputs.endpoint === "contracts") { console.log("Options Contracts"); 
        return `${baseUrlV3}/reference/options/${inputs.endpoint}?underlying_ticker=${inputs.symbol}`
        // underlying_ticker=NVDA&contract_type=call&expiration_date=2024-10-02&as_of=2024-10-01&strike_price=129&expired=false&order=desc&limit=1000&sort=ticker&apiKey=
    }
    // Indices
